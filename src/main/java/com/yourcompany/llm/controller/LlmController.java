...
@Slf4j @RestController @RequestMapping("/api/llm") @RequiredArgsConstructor @CrossOrigin(origins = "*")
public class LlmController {
    ...
    public CompletableFuture<ResponseEntity<LlmResponse>> generateText(@Valid @RequestBody LlmRequest request) {
        ...
        return llmService.generateText(request).thenApply(response -> {
            ...
            LlmResponse errorResponse = LlmResponse.error("llama3.2",
    ...
    public CompletableFuture<ResponseEntity<LlmResponse>> chatCompletion(@Valid @RequestBody LlmRequest request) {
        ...
        return llmService.chatCompletion(request).thenApply(ResponseEntity::ok).exceptionally(throwable -> {
    ...
    public ResponseEntity<Map<String, Object>> getModels() {
        Map<String, Object> response = Map.of("models", Map.of("name", "llama3.2", "provider", "vllm", "maxTokens",
                8192, "description", "Meta Llama 3.2 model running on vLLM"), "timestamp", LocalDateTime.now());
    ...
    public ResponseEntity<Map<String, Object>> healthCheck() {
        Map<String, Object> health = Map.of("status", "UP", "service", "vLLM Llama 3.2 API", "version", "1.0.0",
                "model", "llama3.2", "timestamp", LocalDateTime.now());
    ...
    public CompletableFuture<ResponseEntity<Map<String, Object>>> detailedHealthCheck() {
        return llmService.checkVllmHealth().thenApply(healthStatus -> {
            Map<String, Object> health = Map.of("status", healthStatus, "service", "vLLM Llama 3.2 API", "vllm_status",
    ...
    public ResponseEntity<Map<String, Object>> handleException(Exception e) {
        ...
        Map<String, Object> error = Map.of("error", "Internal Server Error", "message", e.getMessage(), "timestamp",
                LocalDateTime.now());

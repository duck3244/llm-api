...
public class LlmServiceImpl implements LlmService {
    ...
    public CompletableFuture<String> checkVllmHealth() {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                long healthyCount = enabledServers.stream()
                    .mapToLong(server -> {
                        try {
                            // 간단한 포트 체크
    ...
    private LlmRequest convertToChat(LlmRequest request) {
        LlmRequest chatRequest = new LlmRequest();
        chatRequest.setModel("llama3.2");
